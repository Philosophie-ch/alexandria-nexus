...
| `.sortable_columns(&[...])` | Whitelists columns for `?sort_by=col&sort_dir=asc\|desc` on list endpoints; supports multi-column sort via comma-separated values (e.g. `?sort_by=volume,number,start_page&sort_dir=desc,desc,asc`); missing directions default to `asc`; rejects unlisted columns with 400 | All 8 entities in `composition/mod.rs` |
