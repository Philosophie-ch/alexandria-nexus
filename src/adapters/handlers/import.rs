...
use crate::adapters::import::{
    parse_authors_csv, parse_bibitem_notes_csv, parse_bibitem_refs_csv, parse_bibitems_csv,
    parse_institutions_csv, parse_journals_csv, parse_keywords_csv, parse_name_variants_csv,
    parse_publishers_csv, parse_schools_csv, parse_series_csv,
};
...
pub async fn import_authors(
    ...
    let batch_lookup = state.author_batch_lookup();
    let syncer = state.sequence_syncer();
...
pub async fn import_journals(
    ...
    let batch_lookup = state.journal_batch_lookup();
    let syncer = state.sequence_syncer();
...
pub async fn import_publishers(
    ...
    let batch_lookup = state.publisher_batch_lookup();
    let syncer = state.sequence_syncer();
...
pub async fn import_institutions(
    ...
    let batch_lookup = state.institution_batch_lookup();
    let syncer = state.sequence_syncer();
    let result =
        import::import_institutions(&state.institution_ds, &batch_lookup, &syncer, rows, errors)
            .await?;
    Ok(Json(result))
}
...
pub async fn import_schools(
    ...
    let batch_lookup = state.school_batch_lookup();
    let syncer = state.sequence_syncer();
    let result =
        import::import_schools(&state.school_ds, &batch_lookup, &syncer, rows, errors).await?;
    Ok(Json(result))
}
...
pub async fn import_series(
    ...
    let batch_lookup = state.series_batch_lookup();
    let syncer = state.sequence_syncer();
    let result =
        import::import_series(&state.series_ds, &batch_lookup, &syncer, rows, errors).await?;
    Ok(Json(result))
}
...
pub async fn import_keywords(
    ...
    let batch_lookup = state.keyword_batch_lookup();
    let syncer = state.sequence_syncer();
    let result =
        import::import_keywords(&state.keyword_ds, &batch_lookup, &syncer, rows, errors).await?;
    Ok(Json(result))
}
...
pub async fn import_author_name_variants(
    ...
    let variant_store = state.name_variant_store();
    let result =
...
pub async fn import_bibitems(
    ...
    let junction_store = state.bibitem_junction_store();
    let ref_store = state.reference_store();
    let syncer = state.sequence_syncer();
...
pub async fn import_bibitem_refs(
    ...
    let refs_store = state.bibitem_refs_store();
    let id_store = state.reference_store();
...
pub async fn import_bibitem_notes(
    ...
    let notes_store = state.bibitem_notes_store();
    let key_store = state.reference_store();
    let result = import::import_bibitem_notes(&notes_store, &key_store, rows, errors).await?;
    Ok(Json(result))
}
